...
public class MyTv {
	...
	public void setChannel(int channel) {
		...
		// (재사용성 확보와 중복 개발을 회피하기 위하여ㆍ전체 시스템 차원과 단위 시스템 차원의 공통부분을 식별하여 이에 대한 상세 명세를 작성할 수 있다.5)
		if( channel > MAX_CHANNEL || channel < MIN_CHANNEL || isPowerOn == false ) {	//!this.isPowerOn
			return;
		}
		
		// 4.현재 채널을 이전 채널에 저장한다.
		// (개발할 응용소프트웨어의 전반적인 기능과 구조를 이해하기 쉬운 크기로 공통 모듈을 설계할 수 있다.5)
		this.prevChannel = this.channel;	// prevChannel = this.channel;
	...
	// 7.현재 채널을 이전 채널로 변경한다
	// (개발할 응용소프트웨어의 전반적인 기능과 구조를 이해하기 쉬운 크기로 공통 모듈을 설계할 수 있다.10)
	public void gotoPrevChannel() {
		setChannel(this.prevChannel);	// setChannel(prevChannel);
	}
